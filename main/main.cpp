...
void uart_communication_task(void *pvParameters) {
  ...
  /* ##### USER MAIN LOOP ##### */
  while (1) {
    ESP_LOGI(TAG_UART_SENSOR, "--- START FIRST LINE OF LOOP ---");
    ...
    ESP_LOGI(TAG_UART_SENSOR, "--- BACK TO THE FIRST LINE OF LOOP ---");
    vTaskDelay(pdMS_TO_TICKS(5000));
  }
  /* ##### USER MAIN LOOP ##### */
}
...
void readCombinedData() {
  ...
  uint8_t commandGetCombinedData[] = {0xFF, 0x01, 0x87, 0x00, 0x00,
                                      0x00, 0x00, 0x00, 0x78};
  const int responseLength = 13;
  uint8_t responseData[responseLength];

  ESP_LOGI(TAG_TASK, "--- [SENDING] command to get combined data ---");
  uart_write_bytes(UART_PORT, commandGetCombinedData,
                   sizeof(commandGetCombinedData));
